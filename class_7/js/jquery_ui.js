$(function(){
    //input을 키보드로 눌렀을때(keypress) 이벤트
    $(".input_area > input[type='text']").keypress(function(event){
        //enter(keycode=13)일 경우와 입력중인 input의 value(입력값이) 있을때의 경우가 동시에 만족할 경우 실행 
        if(event.keyCode == 13 && $(this).val().length !=0) {
            ...
            //입력된 순간의 현재시간 구하기
            var _date = new Date(); //Date객체(pc의 시각관련 정보를 담고있는 오브젝트)
            ...
            setTimeout(function(){ //setTimesout을 사용한 이유는 애니메이션 관련 클래스 사용시 동일 시점에 발생하면 애니메이션이 작동하지 않아 딜레이를 주기쥐해 사용함
