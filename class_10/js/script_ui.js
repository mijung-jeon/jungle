...
function init(){
    ...
    $(".fullpage").fullpage({
        ...
        afterLoad:function(name, index){//해당화면에 도착시 
    ...
    $("#section0 .link-wrap .link-unit .btn").mouseover(function(){
        ...
        $("#section0 .bg-wrap .bg").eq(_index).addClass("on");//같은 순번의 배경 on 클래스 추가
    });
    ...
    $("#section2 .link-wrap .link-unit .btn").mouseover(function(){
        ...
        var _posReverse = 100 - _pos[_index]//좌표값 배열에서 index로 값을 찾고 그 값을 100기준으로 반전
    ...
    function checkPos(){
        var _tpos = $("#section2 .wave-wrap .unit").css("transform").split(",")[5]; //파도(unit)에 대한 transform값(matrix)중에서 5번째 값만 가져옴
        _tpos = _tpos.replace(")","")// 값뒤의 ")"부분을 replace를 통해 제거
        var _th = $("#section2 .wave-wrap").height();//백분율로 표현하기 위한 전체 높이값을 알아냄
        var _per = _tpos / _th;//현재갑 / 전체값
        var _per1 = 100 - Math.round(_per * 100); //벡분율로 표현 후 다시 100기준으로 반전
    ...
    $("#section3 .photo-wrap > ul").slick({//slick jquery 플러그인
        ...
        variableWidth:true,//이미지 크기만큼 보여줌 (겹칩현상X)
        centerMode:true//기본 왼쪽 정렬이지만 가운데 정렬로 면경 시켜줌
    }).on("afterChange", function(event, slick, current){//슬라이드 한번 이동시 발생회는 이벤트(current인자: 순번(index))
        
        $("#section3 .txt-wrap > ul > li").removeClass("on");//li전체의 on클래스를 삭제
        $("#section3 .txt-wrap > ul > li").eq(current).addClass("on");//해당 index의 li에 on클래스 추가
    })

    //portfolio cursor 효과
    document.addEventListener("mousemove", function(event){
        var pw = $(".photo-wrap").position().top;//커서의 부모인 Photo-wrap의 상단 ㄱ오간값 구함(공간값이 커서와 마우스 포인터의 차이를 없앰)
    ...
    $("#section4 .input-wrap input[type=text").on("textInput", function(event){//input의 키프레스 이벤트
        ...
        //rgb에 쓰일 랜덤값(3개) 구함
        var _r = Math.floor(Math.random()*256);//Math.random:랜덤값, Math.floor:소숫점 아래 버리기
        ...
        .on("animationend",function(){//css에서 만든 애니메이션이 끝나는 시점에서 이벤트 발생
            $(this).remove();//자신을 삭제 
        })
